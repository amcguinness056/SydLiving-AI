...
import { Maximize, Minimize, Clock, Navigation, Flame } from 'lucide-react';
  ...
  return (
    <div className={`w-full h-full relative overflow-hidden bg-slate-200 dark:bg-slate-950 ${isSettingWorkplace ? 'cursor-crosshair' : ''}`}>
      ...
      >
        ...
        {showHeatmap && <HeatmapLayer hub={activeHub?.name || "Martin Place"} />}

        {/* Dynamic Isochrone Reach Rings centered on active Hub */}
        {!showHeatmap && activeHub && (
          <>
            {maxCommuteMins >= 15 && (
              <Circle
      ...
      <div className="absolute top-4 left-4 z-40 flex items-center gap-1 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md p-1 rounded-2xl shadow-lg border border-white/60 dark:border-slate-800">
        <button
          onClick={() => setShowHeatmap(false)}
          className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
            !showHeatmap ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 ${
            showHeatmap ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'
          }`}
        >
      ...
      {/* Legend Overlay */}
      {showHeatmap ? (
        <div className="absolute bottom-4 left-4 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border border-white/60 dark:border-slate-800 px-3.5 py-2.5 rounded-2xl shadow-lg flex items-center gap-3 text-xs font-semibold text-slate-700 dark:text-slate-200">
